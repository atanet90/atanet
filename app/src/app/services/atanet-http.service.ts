...
interface ConstructorType { new(): any; }
...
export class AtanetHttpService {
  ...
  public async delete<T>(uri: string, type: { new(): T }, handleError: boolean = true): Promise<T> {
  ...
  public async get<T>(uri: string, type: { new(): T }, handleError: boolean = true): Promise<T> {
    const combinedUri = this.baseUri + uri;
    const resultObject = await this.handleRequest(
  ...
  public async getArray<T>(uri: string, type: { new(): T }, handleError: boolean = true): Promise<T[]> {
    const combinedUri = this.baseUri + uri;
    const resultObject = await this.handleRequest(
      () => this.httpClient.get(combinedUri),
      type,
  ...
  public async post<T>(uri: string, body: any, type: { new(): T }, handleError: boolean = true): Promise<T> {
    const combinedUri = this.baseUri + uri;
    const resultObject = await this.handleRequest(
  ...
  public async handleRequest<T>(
    ...
    isArray: boolean,
    handleError: boolean = true): Promise<T | T[]> {
