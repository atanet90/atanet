    ...
    public class UserService : IUserService
    {
        ...
        public IList<UserWithScoreDto> GetUsersSortedByScore()
        {
            ...
            if (!this.scoreService.Can(AtanetAction.ViewUserProfile, currentUserId))
            {
                ...
                throw new ApiException(this.apiResultService.BadRequestResult($"Must have a score greater than {minScore} to view user profiles."));
            }

            var topEntries = int.Parse(Environment.GetEnvironmentVariable("SCOREBOARD_ENTRIES"));
            return this.scoreService.GetUsersByScoreQuery()
                .OrderByDescending(x => x.Score)
                .Take(topEntries).ToList();
        }

        public bool IsLowScoreUser(long userId) =>
            this.scoreService.CalculateUserScore(userId) < UserService.LowScoreUser;
        ...
        {
            var subject = this.GetRequiredClaim("sub");
        ...
        private long CreateUser()
        {
            ...
            {
                var user = new User();
                user.Email = this.GetRequiredClaim("email");
                user.Subject = this.GetRequiredClaim("sub");
                user.Picture = this.pictureService.GetAtanetPicture(unitOfWork);
                var userRepository = unitOfWork.CreateEntityRepository<User>();
